...
// Copyright (c) 2011-2014 LeaderCoin Developers.
...
uint256 hashGenesisBlock("0x8833721ad3785a1d8e234d1c3baf88f6135937e92a39be1379abd3c9d2399325");
...
const string strMessageMagic = "LeaderCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if(nHeight >=8001  && nHeight <=13000) {
        nSubsidy = 1000 * COIN;
        }
    else if(nHeight >= 13001 && nHeight <=18000) {
        nSubsidy = 600 * COIN;
        }
    else if(nHeight >= 18001 && nHeight <=23000) {
        nSubsidy = 400 * COIN;
        }
    else if(nHeight >= 23001 && nHeight <=30000) {
        nSubsidy = 300 * COIN;
        }
    else {
        nSubsidy = 200 * COIN;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal LeaderCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LeaderCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "21 March, 2014 - Cyprus approves privatizations needed for bailout";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1395378000;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1854213;
        ...
        assert(block.hashMerkleRoot == uint256("0x9e5f6b324f6be51fd7737e026efefb8a1fb90c3b98ab2acf183eb0ab7c193a8a"));
...
unsigned char pchMessageStart[4] = { 0xda, 0xc0, 0xbd, 0xd2 };
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // LeaderCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
